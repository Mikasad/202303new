...
extern volatile u16 u16LwipR_BufId;
...
extern uint8_t ipc_char_resolve(uint8_t *Inpara);
/* 扩展变量 ------------------------------------------------------------------*/
/* 私有函数原形 --------------------------------------------------------------*/
...
/**
  ...
  * 说    明: 无
  */
char Recdata[10]= {1,2,3,4,5,6};
char Txdata[10] = {11,22,33,44,55,66};
static err_t tcp_echoserver_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
    char *recdata=0;
    struct tcp_echoserver_struct *es;
    ...
    else if(es->state == ES_ACCEPTED)
    {
        ...
        if(recdata!=NULL)
        {
            ...
            for(char i=0; i<p->len; i++)
            {
                ...
                u8LwipR_Buf[u16LwipR_BufId] = recdata[i];
                u16LwipR_BufId ++;

                if(u16LwipR_BufId >= 1024)
                {
                    u16LwipR_BufId = 0;
                }
...
/**
  * @brief  TCP服务器端向指定IP设备发送数据
  * @para   data
  */
void TCP_Server_Send(u8* data, u8 length)
{
    struct tcp_pcb *cpcb;
