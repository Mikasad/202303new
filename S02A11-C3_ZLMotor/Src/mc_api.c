...
{
    if(pCtrlPar[M1].SetVelMotor != pCtrlPar[M1].LastSetVelMotor)
    {
        pCtrlPar[M1].LastSetVelMotor = pCtrlPar[M1].SetVelMotor;
        MCI_ExecSpeedRamp( pMCI[M1], hFinalSpeed, hDurationms );//中菱电机和大澳电机的方向不一致
    }
    else if(pCtrlPar[M1].SetVelMotor == 0)
    {
        pCtrlPar[M1].LastSetVelMotor = pCtrlPar[M1].SetVelMotor;
        MCI_ExecSpeedRamp( pMCI[M1], hFinalSpeed, hDurationms );//中菱电机和大澳电机的方向不一致
    }
...
{
    if(pCtrlPar[M2].SetVelMotor != pCtrlPar[M2].LastSetVelMotor)
    {
        pCtrlPar[M2].LastSetVelMotor = pCtrlPar[M2].SetVelMotor;
        MCI_ExecSpeedRamp( pMCI[M2], hFinalSpeed, hDurationms );//中菱电机和大澳电机的方向不一致
    }
    else if(pCtrlPar[M2].SetVelMotor == 0 )
    {
        pCtrlPar[M2].LastSetVelMotor = pCtrlPar[M2].SetVelMotor;
