...
//            }
            ...
            break;
            /* Not allowed in Motor Enable */
            case NMT_CS_RESET_COMMUNICATION: //82 会修改 协议栈NODE-ID
            {
...
unsigned char MessageType_Check(CO_Data *d,unsigned int Cob_id,unsigned char rtr)
{
    ...
    funcCmd = GET_FUNCION_CODE(Cob_id);
	  if(funcCmd == 0)//如果操作码等于0，OTA中会收到一帧 0x21  01 00 的数据，获取功能码的时候将nodeID>>7 ,导致0x21>>7 = 0
		{
			if(Cob_id == 0) //需要判断是不是Cob_id也等于0.否则在给混合驱动板升级的时候会使从站进入操作态，开启了PDO
			{
				funcCmd = 0;//如果Cob_id的等于0，那么才确认为NMT指令
			}
			else
			{
				funcCmd = 0xF;//如果不等于，那就是一条无效命令
			}
		}
    switch(funcCmd)
    {
    ...
    /* rPDO Cob_ID */
    {
        ...
        if(offset)
        {
            while((offset <= lastIndex) && (i < 4))
            {
                ...
                {
                    *(unsigned int*)d->objdict[offset].pSubindex[1].lpParam = (cobID[i] + nodeId);//|0x80000000上电RPDO默认处于失能状态
                }
        ...
        if( offset ) while ((offset <= lastIndex) && (i < 4))
            {
                if((*(unsigned int*)d->objdict[offset].pSubindex[1].lpParam == cobID[i] + d->Node_ID)||(d->Node_ID==0xFF))
                {
                    *(unsigned int*)d->objdict[offset].pSubindex[1].lpParam = (cobID[i] + nodeId);//|0x80000000上电TPDO默认处于失能状态
                }
