...
/**
  ...
  *
  */
typedef uint32_t ( *PWMC_OverCurr_Cb_t )( PWMC_Handle_t * pHandle );
...
/**
  ...
  *
  */
typedef uint32_t ( *PWMC_RLDetectSetDuty_Cb_t )( PWMC_Handle_t * pHandle, uint16_t hDuty );
...
struct PWMC_Handle
{
  ...
  uint8_t   Sector;                                     /**< the space vector sector number */
	uint8_t		SectorUser_defined;													/**< 用于缓存扇区 用户自定义*/
	uint8_t		PreSectorUser_defined;											/**< 用于缓存上一次扇区 用户自定义*/
	uint8_t		PrePreSectorUser_defined;										/**< 用于缓存上上一次扇区 用户自定义*/
	uint8_t   DistinguishingMotorNumber;								  /**< 用于区分电机号 用户自定义*/
  uint16_t  lowDuty;
  uint16_t  midDuty;
  uint16_t  highDuty;
...
uint32_t PWMC_CheckOverCurrent( PWMC_Handle_t * pHandle );
...
uint32_t PWMC_RLDetectionModeSetDuty( PWMC_Handle_t * pHandle,
                                      uint16_t hDuty );
