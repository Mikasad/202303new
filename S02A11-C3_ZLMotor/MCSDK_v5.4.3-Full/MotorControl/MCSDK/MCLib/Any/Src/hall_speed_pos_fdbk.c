...
#define CCER_CC1E_Reset             ((uint16_t)0xFFFE)
 ...
 void HALL_Init_Electrical_Angle( HALL_Handle_t * pHandle );
    ...
    switch ( pHandle->HallState )
    {
      case STATE_5:
        ...
        else
        {
					...
					if(DEFAULT_TARGET_SPEED_UNIT>0)
					{
						pHandle->Direction = POSITIVE;
						pHandle->MeasuredElAngle = pHandle->PhaseShift;
					}
...
//                                              S16_60_PHASE_SHIFT / 2 );
			 	...
			 	if(DEFAULT_TARGET_SPEED_UNIT>0)
				{
					pHandle->Direction = POSITIVE;
					pHandle->MeasuredElAngle = ( int16_t )( pHandle->PhaseShift + S16_120_PHASE_SHIFT
																								+ S16_60_PHASE_SHIFT + S16_60_PHASE_SHIFT / 2);
				}
				else 
				{
					pHandle->Direction = NEGATIVE;
...
//	HAL_Init_Electrical_Angle = pHandle->MeasuredElAngle ;
