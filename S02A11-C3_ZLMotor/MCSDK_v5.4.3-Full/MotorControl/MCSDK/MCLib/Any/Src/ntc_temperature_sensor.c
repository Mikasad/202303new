...
uint32_t NTC_SetFaultState( NTC_Handle_t * pHandle );
...
/**
  ...
  */
__weak uint32_t NTC_SetFaultState( NTC_Handle_t * pHandle )
{
  uint32_t hFault;
...
/**
  ...
  */
__weak uint32_t NTC_CalcAvTemp( NTC_Handle_t * pHandle )
{
  uint32_t wtemp;
  uint16_t hAux;
...
{
    float wTemp=0,ftemp_R =0;
    ftemp_R = 20*((float)pHandle->hAvTemp_d/(65535-pHandle->hAvTemp_d)); //使用ADC推算当前热敏电阻值
    if ( pHandle->bSensorType == REAL_SENSOR )
    {
        if(pHandle->hAvTemp_d <9010) //100°-140° 不要超过140°
        {
            wTemp = (float)(156.12f - 19.091f*ftemp_R); //使用热敏电阻值推算当前温度
        }
        else if(pHandle->hAvTemp_d <19618) //70°-100°
        {
            wTemp = (float)(115.71f - 5.722f*ftemp_R)-2;
        }				
				else if(pHandle->hAvTemp_d <37474) //40°-70°
        {
            wTemp = (float)(81.04f - 1.649f*ftemp_R)-3;
        }				
				else if(pHandle->hAvTemp_d <58054) //0°-40°
        {
            wTemp = (float)(42.30f - 0.310f*ftemp_R);
        }
				else if(pHandle->hAvTemp_d <63869) //-30°-0°
        {
            wTemp = (float)(3.32f - 0.049f*ftemp_R);
        }
    }
    else
    {
...
/**
  ...
  */
__weak uint32_t NTC_CheckTemp( NTC_Handle_t * pHandle )
{
  return ( pHandle->hFaultState );
}
