...
__weak uint16_t PWMC_SetPhaseVoltage( PWMC_Handle_t * pHandle, alphabeta_t Valfa_beta )
{
  ...
  wX = wUBeta;
  wY = ( wUBeta/ 2 + wUAlpha/ 2 ) ;
  wZ = ( wUBeta/ 2 - wUAlpha / 2) ;
  ...
  if ( wY < 0 )
  {
    if ( wZ < 0 )
    {
      pHandle->Sector = SECTOR_5;
			pHandle->SectorUser_defined = SECTOR_5;
      ...
      if ( wX <= 0 )
      {
        pHandle->Sector = SECTOR_4;
				pHandle->SectorUser_defined = SECTOR_4;
      ...
      else /* wX > 0 */
      {
        pHandle->Sector = SECTOR_3;
				pHandle->SectorUser_defined = SECTOR_3;
  ...
  else /* wY > 0 */
  {
    if ( wZ >= 0 )
    {
      pHandle->Sector = SECTOR_2;
			pHandle->SectorUser_defined = SECTOR_2;
      wTimePhA = ( int32_t )( pHandle->PWMperiod ) / 4 + ( ( wY - wZ ) / ( int32_t )262144 );
      wTimePhB = wTimePhA + wZ / 131072;
      wTimePhC = wTimePhA - wY / 131072;
      pHandle->lowDuty = wTimePhB;
      ...
      if ( wX <= 0 )
      {
        pHandle->Sector = SECTOR_6;
				pHandle->SectorUser_defined = SECTOR_6;
        wTimePhA = ( int32_t )( pHandle->PWMperiod ) / 4 + ( ( wY - wX ) / ( int32_t )262144 );
        wTimePhC = wTimePhA - wY / 131072;
        wTimePhB = wTimePhC + wX / 131072;
      ...
      else /* wX > 0 */
      {
        pHandle->Sector = SECTOR_1;
				pHandle->SectorUser_defined = SECTOR_1;
        wTimePhA = ( int32_t )( pHandle->PWMperiod ) / 4 + ( ( wX - wZ ) / ( int32_t )262144 );
        wTimePhB = wTimePhA + wZ / 131072;
        wTimePhC = wTimePhB - wX / 131072;
        pHandle->lowDuty = wTimePhA;
	...
	if(wTimePhA >= 0 && wTimePhA <= pHandle->PWMperiod)
	{
		pHandle->CntPhA = ( uint16_t )wTimePhA;
	}
	
	if(wTimePhB >= 0 && wTimePhB <= pHandle->PWMperiod)
	{
		pHandle->CntPhB = ( uint16_t )wTimePhB;
	}
	
	if(wTimePhC >= 0 && wTimePhC <= pHandle->PWMperiod)
	{
...
__weak uint32_t PWMC_CheckOverCurrent( PWMC_Handle_t * pHandle )
{
...
/**
  ...
  */
__weak uint32_t PWMC_RLDetectionModeSetDuty( PWMC_Handle_t * pHandle, uint16_t hDuty )
{
