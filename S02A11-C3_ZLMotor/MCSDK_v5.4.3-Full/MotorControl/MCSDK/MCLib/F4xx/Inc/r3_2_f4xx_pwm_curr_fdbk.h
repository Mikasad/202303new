...
typedef const struct
{
  ...
  uint16_t Tw;                    /*!< It is used for switching the context
                                       ...
                                       the counter crest and ADC latest trigger它用于在双MC中切换上下文。它包含计数器峰值和ADC最新触发器之间的最大延迟(以计数器刻度表示)  
                                       */
...
uint32_t R3_2_IsOverCurrentOccurred( PWMC_Handle_t * pHandle );
...
/**
  ...
  */
uint32_t R3_2_RLDetectionModeSetDuty( PWMC_Handle_t * pHandle , uint16_t hDuty );
