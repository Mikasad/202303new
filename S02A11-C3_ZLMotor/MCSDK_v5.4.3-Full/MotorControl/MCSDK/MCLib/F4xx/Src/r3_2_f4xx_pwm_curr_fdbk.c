...
__STATIC_INLINE uint32_t R3_2_WriteTIMRegisters( PWMC_Handle_t * pHdl, uint16_t hCCR4Reg );
...
/**
  ...
  */
uint8_t sectorM1 = 0,beforsectorM1 = 0,prebeforsectorM1 = 0;
uint8_t sectorM2 = 0,beforsectorM2 = 0,prebeforsectorM2 = 0;
__weak void R3_2_GetPhaseCurrents( PWMC_Handle_t * pHdl, ab_t* pStator_Currents )
{
    ...
    uint16_t hReg2;
    uint8_t bSector;
    static uint8_t LostPhaseDividerCounter = 0;
    ...
    pHandle->_Super.Ic = -pStator_Currents->a - pStator_Currents->b;
		...
		LostPhaseDividerCounter++;
		if(LostPhaseDividerCounter == 2) /*50us 2分频变为 100us执行一次*/
		{
			LostPhaseDividerCounter = 0;
      Missing_phase_error_code[M1] = DefaultPhaseCheck(M1); 
		}
		else
		{
			 Missing_phase_error_code[M2] = DefaultPhaseCheck(M2);
		}
...
__weak void R3_2_SwitchOnPWM( PWMC_Handle_t * pHdl )
{
    ...
    LL_TIM_EnableAllOutputs( TIMx );
    LL_TIM_EnableAutomaticOutput( TIMx );//在电机使能之前再自动输出使能
    if ( ( pHandle->pParams_str->LowSideOutputs ) == ES_GPIO )
    {
...
__weak void R3_2_SwitchOffPWM( PWMC_Handle_t * pHdl )
{
    ...
    pHandle->_Super.TurnOnLowSidesAction = false;
    LL_TIM_DisableAutomaticOutput( TIMx );//关闭自动输出使能，初始化打开自动输出使能以后，此处调用 LL_TIM_DisableAllOutputs，但是AOE 又会打开 MOE，在电机使能之前再自动输出使能
...
//    LL_GPIO_ResetOutputPin (pHandle->pParams_str->pwm_en_w_port, pHandle->pParams_str->pwm_en_w_pin);
    ...
    LL_TIM_ClearFlag_UPDATE(TIMx);
//    while ( LL_TIM_IsActiveFlag_UPDATE( TIMx ) == 0 )//这里屏蔽运行一段之间观察
    {}
    LL_TIM_ClearFlag_UPDATE(TIMx);
...
/**
  ...
  *         It means set the sampling point via TIMx_Ch4 value and polarity
  *         ADC sequence length and channels.
  *         And call the WriteTIMRegisters method.为当前与扇区1相关的采样配置ADC。 即通过TIMx_Ch4值和极性ADC序列长度、通道设置采样点。 然后调用WriteTIMRegisters方法。
  * @param  pHdl: handler of the current instance of the PWM component
  * @retval none
  */
...
//         * between offsets 当有可能在PWM周期的中间采样时，总是为所有扇区采样相同的相位(AB被选择)，以避免在偏移之间存在差异时诱发电流不连续  */

//        /* sector number needed by GetPhaseCurrent, phase A and B are sampled wIch corresponds getphaseccurrent所需要的扇区号，A阶段和B阶段的采样是对应的
//         * to sector 4 */
//        pHandle->_Super.Sector = SECTOR_4;
...
/**
  * @brief  It contains the TIMx Update event interrupt 两个电机都调用，所以频率为20K
  * @param  pHandle: handler of the current instance of the PWM component
  * @retval none
  */
__weak void * R3_2_TIMx_UP_IRQHandler( PWMC_R3_2_Handle_t * pHandle)
{
    ...
    if ( ADCInjFlags == LL_ADC_FLAG_JSTRT )//如果ADC注入通道转换开始
    {
        /* ADC conversion is on going on the second motor ADC转换在第二个电机上进行   */
        do
        {
            /* wait for end of conversion 等待转换完成*/
            ADCInjFlags = ADCx_1->SR & (LL_ADC_FLAG_JSTRT|LL_ADC_FLAG_JEOS);
        }
    ...
    {
        /* ADC conversion on the second motor is not yet started  第二个电机上的ADC转换尚未启动  */
        while ( ( TIMx->CNT ) < ( pHandle->pParams_str->Tw ) )
        {
            /* wait for a maximum delay 等待最大延迟 */
        }
    ...
    {
        /* ADC conversion on the second motor is done 在第二个电机上完成ADC转换 */
    }

    /* Disabling trigger to avoid unwanted conversion 禁用触发器以避免不必要的转换  */
    LL_ADC_INJ_StopConversionExtTrig(ADCx_1);
    LL_ADC_INJ_StopConversionExtTrig(ADCx_2);

    /* Set next current channel according to sector  根据扇区设置下一个当前通道*/
    ...
    /* enable ADC trigger source 启用ADC触发源 */
    ...
    /* reset default edge detection trigger 复位默认的边缘检测触发器*/
    pHandle->ADCTriggerEdge = LL_ADC_INJ_TRIG_EXT_RISING;
...
/**
  ...
  */
__weak uint32_t R3_2_IsOverCurrentOccurred( PWMC_Handle_t * pHdl )
{
    PWMC_R3_2_Handle_t * pHandle = (PWMC_R3_2_Handle_t *) pHdl;
...
/**
  ...
  */
uint32_t R3_2_RLDetectionModeSetDuty( PWMC_Handle_t * pHdl, uint16_t hDuty )
{
    PWMC_R3_2_Handle_t * pHandle = ( PWMC_R3_2_Handle_t * )pHdl;
    TIM_TypeDef * TIMx = pHandle->pParams_str->TIMx;
