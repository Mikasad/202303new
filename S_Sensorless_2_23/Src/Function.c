...
/*************************无刷5，6***************************/
	...
	Sensorless[0].Acceleration=5;
	Sensorless[0].Deceleration=5;
...
//		MotorControl[5].PercentOfPWM = 100;
    ...
    MotorControl[6].Deceleration = 5;
		...
		MotorControl[7].Current.MaxValue1 = MOTOR7_CurrentValue(3) ;  //边刷电机
    MotorControl[7].Current.MaxValue2 = MOTOR7_CurrentValue(3.5) ;
    MotorControl[7].Current.MaxValue3 = MOTOR7_CurrentValue(4) ;  //边刷电机
...
/*************************无刷5，6end***************************/
        ...
        PID_Sensorless_PWM_Init(&Sensorless_PID_PWM);		//推杆0的PID
/*	加减速度初始化	*/
    MotorControl[0].Acceleration = 30;
    MotorControl[0].Deceleration = 30;
...
/*无感*/
	Sensorless[0].CountSectorCnt3= 300;//20;
    ...
    if(MotorControl[motornum].Push_Location_model==1)//标定正确，开始执行位置控制
    {
        if(motornum == 0) //50机器滚刷推杆，后期预留滚刷自适应
        {
            if(MotorControl[5].Current.SetValue ==0)	//没有做电流自适应的情况，进行位置控制
            {
                ...
                {
                    MotorControl[motornum].PercentOfPWM = 80;
                }
...
void Motor_Choose(void)
{
	if(Motor_Type==0)                  //滚刷电机选择，0：联谊滚刷电机   1：和泰滚刷电机
	{
		...
		PID_Current_InitStructure[0].hLower_Limit_Output=-8000;
		PID_Current_InitStructure[0].hUpper_Limit_Output=8000;
		PID_Current_InitStructure[0].wLower_Limit_Integral = -8000 * 512;
        PID_Current_InitStructure[0].wUpper_Limit_Integral = 8000 * 512;
	}
	else if(Motor_Type==1)
	{
		...
		PID_Current_InitStructure[0].hLower_Limit_Output=-4000;
