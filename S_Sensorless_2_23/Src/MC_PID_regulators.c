...
PID_Struct_t	 Sensorless_PID_PWM;
...
void PID_PWM_Init(PID_Struct_t *PID_PWM)				/* 推杆PID初始化 */
{
    ...
    PID_PWM->hKp_Gain    = 3000;
    PID_PWM->hKp_Divisor = 64;

    PID_PWM->hKi_Gain = 40;
    PID_PWM->hKi_Divisor = 512;

    PID_PWM->hKd_Gain = KD_GAIN_CURRENT;
    PID_PWM->hKd_Divisor = CURRENT_KDDIV;
    PID_PWM->wPreviousError = 0;

    PID_PWM->hLower_Limit_Output= -PWM_PERIOD;   //Lower Limit for Output limitation
    PID_PWM->hUpper_Limit_Output= PWM_PERIOD ;   //Upper Limit for Output limitation
    PID_PWM->wLower_Limit_Integral = -PWM_PERIOD * CURRENT_KIDIV;
    PID_PWM->wUpper_Limit_Integral = PWM_PERIOD * CURRENT_KIDIV;
    PID_PWM->wIntegral = 0;
    PID_PWM->Ksat = 1;
}

void PID_Sensorless_PWM_Init(PID_Struct_t *Sensorless_PID_PWM)				/* PID初始化 */
{
    Sensorless_PID_PWM->wIntegral = 0;  // reset integral value
    Sensorless_PID_PWM->hReference = 0;
    Sensorless_PID_PWM->hKp_Gain    = 128;  //200
    Sensorless_PID_PWM->hKp_Divisor = 128;

    Sensorless_PID_PWM->hKi_Gain = 5;//50;
    Sensorless_PID_PWM->hKi_Divisor = 2048;//512;

//    Sensorless_PID_PWM->hKd_Gain = KD_GAIN_CURRENT;
//    Sensorless_PID_PWM->hKd_Divisor = CURRENT_KDDIV;
//    Sensorless_PID_PWM->wPreviousError = 0;

    Sensorless_PID_PWM->hLower_Limit_Output= -3500;   //Lower Limit for Output limitation
    Sensorless_PID_PWM->hUpper_Limit_Output= 3500 ;   //Upper Limit for Output limitation
    Sensorless_PID_PWM->wLower_Limit_Integral = -3500 * CURRENT_KIDIV;
    Sensorless_PID_PWM->wUpper_Limit_Integral = 3500 * CURRENT_KIDIV;
    Sensorless_PID_PWM->wIntegral = 0;
    Sensorless_PID_PWM->Ksat = 1;
}
