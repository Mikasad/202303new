...
u8 NeedToCheekPush1[MOTOR_NUM] = {3,0,0,2,2,0,0,0,0,0,0,0,0,0}; //如果该推杆电机需要霍尔错误或者推杆断联检测 0 1 2 9号电机默认都打开检测
		   ...
		   if(NeedToCheekPush1[i]==4)
        {
            if(MotorControl[1].Motor_Start_Stop == 1)
            {
                if(MotorControl[1].PWM_Duty>PUSH_PROTECT_MIN_VAL)
                {
                    ...
                    if(Push_Error_Cheek_Cnt[5] %30000==0)
                    {
                ...
                else if(MotorControl[1].PWM_Duty<-PUSH_PROTECT_MIN_VAL)
                {
                    ...
                    if(Push_Error_Cheek_Cnt_1[1]%30000==0)
                    {
