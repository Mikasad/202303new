...
int main(void)
{
	...
	buf_send[0] = 0x00;
	CANx_SendNormalData(&hcan2,0x702,buf_send,1);
    ...
    while (1)
    {
        ...
        case Wait:
            if(HAL_GetTick()>5000)                                         /*如果在wait状态持续2.5S则跳转至app，上电不升级跳转*/
            {
				...
				BackUp_data[1]=16384;                                      /*16K 大小16384  16384*/
				...
				Flash_Read(ADDR_FLASH_SECTOR_4,OTA_Flag,1);  
				if(OTA_Flag[0] != 0x00000003)
				{					
					 if(APP_Backup(APP_Sector_2,APP_Sector_1,BackUp_data[1],rollback_packnum)==TRUE)   /*APP程序回退 bootloader0从08025000放到800，bootloader1从0x08029000放到0x08004000*/
					{
						if(APP_Backup(APP_Sector_3,APP_Sector_4,BackUp_data[1],rollback_packnum)==TRUE) 
						{
							buf_send[0] = 0x08;
							buf_send[1] = 0x08;
							CANx_SendNormalData(&hcan2,0xFF,buf_send,2);
							OTA_Flag[0] = 0x00000003;//4
						    FlashEraseSector(FLASH_SECTOR_4,1);
							if(WriteUint32_tData_toFlash(OTA_Flag,1,ADDR_FLASH_SECTOR_4)==1)
							{
							   SoftReset();
							}
						}
...
void HAL_CAN_RxCpltCallback(CAN_HandleTypeDef *hcan)
{
    ...
    if(hcan->Instance ==CAN2)
    {
        ...
        if(RxMessage.StdId == 0x602)                   /*为了防止boot升级过程中断电，boot不进行跳转等待升级指令，回复 CANID: 0x20 data: */
        {
			if(RxMessage.Data[0]==0x2F&&RxMessage.Data[1]==0x0B&&RxMessage.Data[2]==0x20&&RxMessage.Data[3]==0x00&&RxMessage.Data[4]==0x01)
            {
                SoftReset();
			}
			else if(RxMessage.Data[0]==0x40&&RxMessage.Data[1]==0x00&&RxMessage.Data[2]==0x30&&RxMessage.Data[3]==0x03)       //2023.4.25新增
			{
				buf_send[0] = 0x43;
				buf_send[1] = 0x00;
				buf_send[2] = 0x30;
				...
				buf_send[6] = 0x41;
				buf_send[7] = 0x48;
				CANx_SendNormalData(&hcan2,0x582,buf_send,8);
			}
        }
    }
    HAL_CAN_Receive_IT(&hcan2, CAN_FIFO0);
