...
/*************************无刷5，6***************************/
    ...
    MotorControl[5].Current.MaxValue4 = MOTOR5_CurrentValue(10.5) ;
		MotorControl[5].Current.CurOffset = 15;		//默认0.3A
...
//		MotorControl[5].PercentOfPWM = 100;
    ...
    MotorControl[6].Deceleration = 5;
		...
		MotorControl[7].Current.MaxValue1 = MOTOR7_CurrentValue(2) ;  //边刷电机
    MotorControl[7].Current.MaxValue2 = MOTOR7_CurrentValue(3) ;
    MotorControl[7].Current.MaxValue3 = MOTOR7_CurrentValue(8) ;  //边刷电机
    MotorControl[7].Current.MaxValue4 = MOTOR7_CurrentValue(8) ;
    MotorControl[7].Current.OFCnt1_T = 0;
    MotorControl[7].Current.OFCnt2_T = 2000;
    MotorControl[7].Current.OFCnt3_T = 20;
    MotorControl[7].Current.OFCnt4_T = 20;
		
		MotorControl[8].Frequency = 10000/(TIM4->PSC+1);
		MotorControl[8].Current.MaxValue1 = MOTOR8_CurrentValue(4) ;  //吸风电机
    MotorControl[8].Current.MaxValue2 = MOTOR8_CurrentValue(15) ;
    MotorControl[8].Current.MaxValue3 = MOTOR8_CurrentValue(20) ;  //吸风电机
    MotorControl[8].Current.MaxValue4 = MOTOR8_CurrentValue(20) ;
    MotorControl[8].Current.OFCnt1_T = 0;
    MotorControl[8].Current.OFCnt2_T = 20000;
    MotorControl[8].Current.OFCnt3_T = 20;
    MotorControl[8].Current.OFCnt4_T = 20;
	TIM4->CCR4= 0;
/*************************无刷5，6end***************************/
...
void Push_Motor_CurSelfAdapt(u8 bldc_num,u8 push_num,u16 hall_max,u16 hall_min)
{
	...
	if(MotorControl[bldc_num].Current.SetValue!=0)
	{
		...
		if(MotorControl[push_num].Hall.HALL_CaptureValue<=(MotorControl[push_num].Limit_Hall_Number-5))
		{
			MotorControl[push_num].Motor_Start_Stop = ENABLE;
			MotorControl[push_num].PercentOfPWM=-80;
		}
		else 
		{
			if(MotorControl[bldc_num].Fault_Flag==0)
			{
				...
				{
					if(f_abs(MotorControl[bldc_num].Current.DeepFilterVAL-MotorControl[bldc_num].Current.SetValue*cur_ratio[bldc_num])>(MotorControl[5].Current.CurOffset+15)*cur_ratio[bldc_num])
					{
						hight_err_cnt++;
...
//							if(MotorControl[push_num].PercentOfPWM>0)
//							{
//								if(MotorControl[push_num].Hall.HALL_CaptureValue<=230)
//								{
//									MotorControl[push_num].PercentOfPWM=0;
//								}
//							}
...
//						}
		...
		if(MotorControl[bldc_num].Speed_Real<-100)         //转起来的情况
		{
			...
			if((Push_Type==0&&MotorControl[push_num].Hall.HALL_CaptureValue>120)||(Push_Type==1&&MotorControl[push_num].Hall.HALL_CaptureValue>250))
			{
				MotorControl[push_num].Inplace=1;
			}
			
			else if((Push_Type==0 && MotorControl[push_num].Hall.HALL_CaptureValue>60 && cur_temp>MotorControl[bldc_num].Current.SetValue_Min)||(Push_Type==1 && MotorControl[push_num].Hall.HALL_CaptureValue>120 && cur_temp>MotorControl[bldc_num].Current.SetValue_Min))
			{
				MotorControl[push_num].Inplace=1;
			}
	...
	else
	{
		...
		{	
			if(Inplacecnt>600)   //6s
			{
				MotorControl[0].Hall.HALL_CaptureValue=0;
				Inplacecnt=0;
			}
			else if((Push_Type==0&&MotorControl[0].Hall.HALL_CaptureValue<35||Push_Type==1&&MotorControl[0].Hall.HALL_CaptureValue<70)&&MotorControl[0].Current.FilterValue<=3)  //8s
			{
				MOTOR1_Inplacecnt++;
...
u16 fan_overspeedount=0;
void Fan_OverSpeed(void)
{
	
	switch (MotorControl[8].PercentOfPWM)
	{
		case 20:
			if(MotorControl[8].Speed_Real>57600)
			{
				fan_overspeedount++;	
			}
			else 
			{
				if(fan_overspeedount>0)
				{
					fan_overspeedount--;
				}
			}
			if(fan_overspeedount>2500)
			{	
		...
		break;
		case 40:
			if(MotorControl[8].Speed_Real>79800)
			{
				fan_overspeedount++;	
			}
			else 
		...
		break;
		case 60:
			if(MotorControl[8].Speed_Real>94000)
			{
				fan_overspeedount++;	
			}
			else 
