...
/* 要擦除内部FLASH的起始地址 */
#define FLASH_USER_START_ADDR   ADDR_FLASH_SECTOR_7     //6
#define FLASH_USER_END_ADDR			ADDR_FLASH_SECTOR_8    //7
...
uint8_t WriteData_toFlash(int32_t *data,uint32_t len,uint32_t address)   //写入之前先擦除
{
	HAL_FLASH_Unlock();
    uint32_t i=0;
    for(i=0; i<len; i++)
        ...
        if(PARAMETER[i].stAttributes.bSaveToFlash ==YSFLASH)
				{
					*PARAMETER[i].lpParam = *(long*)(dataddress);
					dataddress +=4;
					Select_Flag=transbuf[82]; // 校准成功 
					if(Select_Flag==1)
					{					
						MotorControl[0].Adjust_Flag=transbuf[80]; // 校准完成                          
						Push_Type=transbuf[81]; // 推杆类型                                          						                                 
						MotorControl[0].Hall.HALL_CaptureValue_Max=transbuf[83]; //最大hall记录 
					}
				}
	}
}
...
void OTA_Flash_Read(uint32_t addresss,uint32_t *pBuffer,uint32_t bufsize)			//读取flash参数
{
    uint32_t i;
    for(i=0; i<bufsize; i++)
    {
