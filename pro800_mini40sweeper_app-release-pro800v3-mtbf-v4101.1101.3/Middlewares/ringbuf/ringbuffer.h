...
*/
#define RING_BUFFER_SIZE (2*1024U)
 ...
 * @param buffer The ring buffer size.
 */
int32_t ring_buffer_create(ring_buffer_t *buffer,uint32_t size);

/**
 * @brief 当不使用这个队列时候,调用删除
 * @param  buffer 需要删除的队列
 */
...
inline uint8_t ring_buffer_is_full(ring_buffer_t *buffer) {
  return ((buffer->head_index - buffer->tail_index) & (buffer->size - 1) ) == buffer->size ;
}
...
inline ring_buffer_size_t ring_buffer_num_items(ring_buffer_t *buffer) {
  return ((buffer->head_index - buffer->tail_index) & (buffer->size - 1) );
}
