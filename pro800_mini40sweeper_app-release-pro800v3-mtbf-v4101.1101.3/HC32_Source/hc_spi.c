...
*������	: hc_spi_recv
*��	 ��	��SPI����
*��  �� : pu8TxBuf:���ͻ��棬pu8RxBuf:���ջ��棬u32Size�����ݳ���
*����ֵ : 
 *         - LL_OK:                 No errors occurred
...
{
    return SPI_TransReceive(BSP_SPI_UNIT, pu8TxBuf, pu8RxBuf, u32Size, BSP_SPI_TIMEOUT);
}
...
*����ֵ : ��
******************************************************************************/
extern void GetID_90(uint8_t *fp_mID, uint8_t *fp_dID);
extern void GetID_9F(uint8_t *fp_mID, uint8_t *fp_type, uint8_t *fp_capacity);
extern int WriteData(uint32_t f_destAddr, uint8_t *fp_data, uint32_t f_len);
extern void ReadDataBytes(uint32_t f_srcAddr, uint8_t *fp_dataBuf, uint32_t f_len);
#define TEST_ADDR   0x2000
void hc_spi_test(void)
{
	uint8_t i = 0;
	uint8_t spiRxBuf[64] = {0};
	
	printf("Read Flash Id Test:\r\n");
	printf("Init_Val:%02X %02X %02X %02X\r\n",spiRxBuf[0],spiRxBuf[1],spiRxBuf[2],spiRxBuf[3]);
	GetID_90(&spiRxBuf[0], &spiRxBuf[1]);
	printf("GetID_90:%02X %02X\r\n",spiRxBuf[0],spiRxBuf[1]);
	GetID_9F(&spiRxBuf[0], &spiRxBuf[1], &spiRxBuf[2]);
	printf("GetID_9F:%02X %02X %02X\r\n",spiRxBuf[0],spiRxBuf[1],spiRxBuf[2]);
	
	printf("Read/Write Flash at Addr:0x%04X\r\n",TEST_ADDR);
	printf("Read Flash Before Write:\r\n");
	ReadDataBytes(TEST_ADDR, spiRxBuf, sizeof(spiRxBuf));
	for (i = 0; i < sizeof(spiRxBuf); i++)
		printf("%02X ",spiRxBuf[i]);
	printf("\r\n");
	for (i = 0; i < sizeof(spiRxBuf); i++)
		spiRxBuf[i] = i;
	WriteData(TEST_ADDR, spiRxBuf, sizeof(spiRxBuf));
	printf("Read Flash After Write:\r\n");
