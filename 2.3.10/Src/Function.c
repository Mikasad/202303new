...
void DisplayErrLed(void)
{
    ...
    else
    {
        ...
        {
        case PUSH_MOTOR1_OVER_CUR: //推杆1过流闪烁1次
            LEDSet(LED_OFF,LED_ON,1,0,LED_HIGH_SPEED);

            break;

        case PUSH_MOTOR2_OVER_CUR: //推杆2过流闪烁2次
            LEDSet(LED_OFF,LED_ON,2,0,LED_HIGH_SPEED);

            break;

        case PUSH_MOTOR3_OVER_CUR: //推杆3过流闪烁3次
            LEDSet(LED_OFF,LED_ON,3,0,LED_HIGH_SPEED);

            break;

        case PUSH_MOTOR4_OVER_CUR: //推杆4过流闪烁4次
...
Input   :No
Output  :No
Explain : 100us，霍尔检测需要确定其最大行程，断线保护需要确定其累计3s的电流值
------------------------------------------------*/
...
Input   :No
Output  :No
Explain :其中的缺相时的电流比值的数值需要根据不同的电机进行测试
------------------------------------------------*/
#define ac 0
#define ab 1
#define bc 2
...
void Motor5_Default_Phase_Cheek(void)
{
   if(Motor_Type<2)
  {
    ...
    {
        M5_100us_ac_ab_bc_cur[ab] = 0;                                  //没有达到上述条件则所有电流清0
        M5_100us_ac_ab_bc_cur[ac] = 0;
        M5_100us_ac_ab_bc_cur[bc] = 0;
...
{
  if(Motor_Type<2)   //选择无刷电机的时候才进行检测
  {
	  if(Motor_Type==0)
	  {
...
* Input           : bldc_num  push_num  hall_max  hall_min									
* Output          : None
* Return          : None
*************************/
...
void BLDC_Stuck_Chk(void)			/* 1ms循环里 */
{
    if(GetMotorSpeed(Motor5)==0&&ABS(MotorControl[5].PWM_Duty)>4000&&MotorControl[5].Motor_Start_Stop==1)	//BLDC1带载时缺相，电流为0
    {
        ...
        else if(MotorControl[5].Current.FilterValue > 700)  //堵转电流>7A
        {
            M5stuck_check_cnt++;//堵转计数
        }

        if(M5phase_check_cnt>PHASE_ERR_MAX)  //时间达到5s
        {
        ...
        if(M5stuck_check_cnt>SUTCK_ERR_MAX)  //时间达到3s
        {
    ...
    if(GetMotorSpeed(Motor6)==0&&ABS(MotorControl[6].PWM_Duty)>4000&&MotorControl[6].Motor_Start_Stop==1)	//BLDC1带载时缺相，电流为0
    {
        ...
        else if(MotorControl[6].Current.FilterValue > 700)  //堵转电流>7A
        {
...
*Description   		:BLDC1失速报警  
* Input           : None
* Output          : None
* Return          : None
*************************/
void BLDC1_OverSpdChk(void)  //在2ms中
{
   if(Motor_Type<2)
  {
...
*Description   		:BLDC2失速报警  
* Input           : None
* Output          : None
* Return          : None
*************************/
void BLDC2_OverSpdChk(void)  //在2ms中
{
 if(Motor_Type<2)
 { 
    static u16 speed_err_cnt = 0;
...
/*************************
*Function Name 		:Push_OverRunChk,失能计数清0放在2ms中
*Description   		:推杆动作时间限制
* Input           : push 1-4:	0关闭 1开启	timer:0-256 多少秒后停止						
* Output          : None
* Return          : None		
*************************/
u8 timer_cnt[4]=0;
u8 OverRunFlag=0;
void Push_OverRunChk(u8 push1,u8 timer0,u8 push2,u8 timer1,u8 push3,u8 timer2,u8 push4,u8 timer3)  //之前不报因为在DisplayErrLed()会对全局错误1进行清除因此没报。
{
...
void Push_Motor_Inplace(void)   //周期10ms
{
	...
	if(MotorControl[1].Direction==CW)  //拉伸
	{
		MotorControl[1].Fang_xiang=1;
	}
	else if(MotorControl[1].Direction==CCW) //缩回
	{
		MotorControl[1].Fang_xiang=2;
	}
	else if(MotorControl[1].Direction==2) //停止
	{
