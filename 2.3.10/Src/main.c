...
uint16_t  Mix_Motor_StartStop=0;
...
uint8_t CAN1_Send_Msg(uint32_t stdId,uint32_t Ide,uint32_t Rtr,uint8_t* msg,uint8_t len)
{
    ...
    for(i=0; i<len; i++)
    {
	    ...
	    for(i=0; HAL_CAN_Transmit(&hcan2,100)!=HAL_OK && i<3; i++) {}
    if(i>=3)
    {
    }
...
//    __HAL_USART_ENABLE_IT(&huart3,USART_IT_RXNE );    //开启串口中断处理函数
    ...
    Init_Drive_Para();
    Program_Version_Init(&ProgramVersion,'5',2,3,10);    //软件版本1.0.2   //1.0.3 见Readme
    ...
    while (1)
    {
        ...
        if(enableallmotor == 1)			//使能所有电机
        {
            ...
            for(u8 i = 0 ; i <= 13; i++)
            {
                ...
                MotorControl[i].Motor_Start_Stop = 1;            //使能所有电机
                if(errormotor5 > 10)
                {
                    MotorControl[5].Motor_Start_Stop = 0;       //超过一定错误计数失能所有电机
                }
                if(errormotor6 > 10)
                {
...
Explain :No
------------------------------------------------*/
void SystemClock_Config(void)   //APB1:84Mhz   APB2:168Mhz
{
...
static void MX_CAN2_Init(void)
{
    ...
    hcan2.Init.SJW = CAN_SJW_1TQ;      // BTR-SJW 
    hcan2.Init.BS1 = CAN_BS1_6TQ;      // BTR-TS1 
    hcan2.Init.BS2 = CAN_BS2_7TQ;      // BTR-TS1 
    hcan2.Init.TTCM = DISABLE;         // MCR-TTCM  
    hcan2.Init.ABOM = ENABLE;          // MCR-ABOM  
    hcan2.Init.AWUM = ENABLE;          // MCR-AWUM  
    hcan2.Init.NART = DISABLE;         // MCR-NART  
    hcan2.Init.RFLM = DISABLE;         // MCR-RFLM  
    hcan2.Init.TXFP = DISABLE;         // MCR-TXFP  
...
//    sFilterConfig.FilterScale = CAN_FILTERSCALE_32BIT; //????????32??
	...
	sFilterConfig.FilterMode = CAN_FILTERMODE_IDLIST;
	sFilterConfig.FilterScale = CAN_FILTERSCALE_32BIT;
    sFilterConfig.FilterIdHigh         = 0x000<<5;        /* 要过滤的ID高位 */
    sFilterConfig.FilterIdLow          = 0x0000;         /* 要过滤的ID低位 */
    sFilterConfig.FilterMaskIdHigh     = 0x000<<5;      /* 过滤器高16位每位必须匹配 */
    sFilterConfig.FilterMaskIdLow      = 0xFFFF;      /* 过滤器低16位每位必须匹配 */
	sFilterConfig.FilterFIFOAssignment=CAN_FilterFIFO0;
	sFilterConfig.BankNumber = 14;
	sFilterConfig.FilterActivation=ENABLE;
	...
	sFilterConfig.FilterMode = CAN_FILTERMODE_IDLIST;
	sFilterConfig.FilterScale = CAN_FILTERSCALE_32BIT;
    sFilterConfig.FilterIdHigh         = 0x77F<<5;        /* 要过滤的ID高位 */
    sFilterConfig.FilterIdLow          = 0x0000;         /* 要过滤的ID低位 */
    sFilterConfig.FilterMaskIdHigh     = 0x77F<<5;      /* 过滤器高16位每位必须匹配 */
    sFilterConfig.FilterMaskIdLow      = 0xFFFF;      /* 过滤器低16位每位必须匹配 */
	sFilterConfig.FilterFIFOAssignment=CAN_FilterFIFO0;
	sFilterConfig.BankNumber = 14;
	sFilterConfig.FilterActivation=ENABLE;
...
/**
  ...
  * @retval None
  */
static void MX_TIM1_Init(void)   //10khz
{
...
/**
  ...
  * @retval None
  */
static void MX_TIM2_Init(void)  //10khz
{
...
/**
  ...
  * @retval None
  */
static void MX_TIM3_Init(void)  //10khz
{
...
/**
  ...
  * @retval None
  */
static void MX_TIM4_Init(void)  //10khz
{
...
/**
  ...
  * @retval None
  */
static void MX_TIM8_Init(void)  //10khz
{
...
/**
  ...
  * @retval None
  */
static void MX_TIM10_Init(void)  //10khz
{
...
/**
  ...
  * @retval None
  */
static void MX_TIM11_Init(void)  //10khz
{
...
//		RED_LED2_TOGGLE;
        ...
        MotorControl[0].Current.GetADCValue = ADC_ConvertedValue[6]-MotorControl[0].Current.offset;               //电流采样+滤波处理
        ...
        VoltVar.AdBuf = ADC_ConvertedValue[10];                   //母线电压采集
        ...
        if(MotorState == RUN&&Motor_Type<2)   //控制无刷电机
        {
            ...
            {
                if(MotorControl[5].Speed_Real<0)         //速度反向电流取正
                {
		...
		else if(MotorState == RUN&&Motor_Type==2)          //切换有感控制有刷电机
		{    
			if(MotorControl[5].Motor_Start_Stop==ENABLE)
            {
			      SetMotorSpeed(5, MotorControl[5].PWM_Duty);
...
/**
  ...
  * @retval None
  */
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)   //50没用到
{
...
//	Push_Motor_Location_Control(1);
    ...
    if(HAL_GetTick()%4 == 0)  //2MS
    {
        ...
        case RUN:   // motor running
            for(u8 i=0; i < MOTOR_NUM; i++)
            {
                ...
                else if(MotorControl[i].Motor_Start_Stop == DISABLE)      //超时保护失能计数清0
                {
                    SetMotorStop(i);
					if(i==0)
					{
            ...
            {
				Mix_Motor_StartStop=MotorControl[13].Motor_Start_Stop<<13|MotorControl[12].Motor_Start_Stop<<12|MotorControl[11].Motor_Start_Stop<<11|MotorControl[10].Motor_Start_Stop<<10|MotorControl[9].Motor_Start_Stop<<9|MotorControl[8].Motor_Start_Stop<<8|MotorControl[7].Motor_Start_Stop<<7
				                    |MotorControl[6].Motor_Start_Stop<<6|MotorControl[5].Motor_Start_Stop<<5|MotorControl[4].Motor_Start_Stop<<4|MotorControl[4].Motor_Start_Stop<<4|MotorControl[3].Motor_Start_Stop<<3|MotorControl[2].Motor_Start_Stop<<2| MotorControl[1].Motor_Start_Stop<<1 |MotorControl[0].Motor_Start_Stop ;
        ...
        case HALL_STUDY_MOTOR5:                         //霍尔学习

            if(First_Enter_STUDY_MOTOR5 == 1)          //一开始将三相PWM关闭
            {
                TIM1->CCR1 = 0;
                TIM1->CCR2 = 0;
                TIM1->CCR3 = 0;
            ...
            HallStudyHandle0();                          //霍尔学习

            if(HALL_Study[0].CommuntionState == 3 )         //霍尔学习完成
            {
                ...
                TIM1->CCR2 = 0;
                TIM1->CCR3 = 0;
                if ( (wGlobal_Flags & HALL5_SENSOR_ERR) == 0 )   //若是发生霍尔错误则跳转至Fault状态，否则进IDLE状态
                {
                    MotorState = IDLE;
                }
                else
    ...
    if(HAL_GetTick()%20 == 0) //10ms 一次 500ms保护
    {
		...
		if(OTA_Cnt_Flag==1)
		{
			if(Erase_Sector(ADDR_FLASH_SECTOR_4,1)==TRUE)
		   {
		    WriteData_toFlash(OTA_Flag,2,ADDR_FLASH_SECTOR_4);    //此时代表升级成功
		    OTA_Cnt_Flag=0;
		   }
		}
    }
